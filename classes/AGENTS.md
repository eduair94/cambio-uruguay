...
| `redditbot/subrules.ts` | el portón: subs que banearon la cuenta (BANNED_SUBS, hardcodeado — un ban es un hecho, no configuración) y subs que prohíben bots **en sus reglas publicadas** (r/CharruaDevs lo dice; r/AskUruguayan NO lo decía y baneó igual, así que leer reglas es necesario y no suficiente). `looksLikeBan` reconoce SUBREDDIT_NOTALLOWED, que Reddit manda dentro de un 200 |
