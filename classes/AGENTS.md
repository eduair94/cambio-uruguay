...
| `gaps/` | questions the site cannot answer → `cluster`(greedy over cosine),`research`(Claude WebSearch with **URLs we verify ourselves**, or Gemini grounded where the model's own retrieval is the proof),`draft`(→ `docs/reddit-gaps/*.md`, **never a page**),`refresh` |
