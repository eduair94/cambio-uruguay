// Fuel consumption in litres per 100 km: what the advert states, and, when it states nothing, what the
// adverts of the same model and engine state.
//
...
// spellings ("16km Por litro", "En ciudad 11km x lt / En Ruta 14km x lt", "6,5 l/100 km"). Sellers
// write km per litre almost always; the directory measures and shows litres per 100 km, so every
// figure is converted once, here, and rounded only at the end.
import { fold } from "./normalize";
import type { CarListing } from "./types";

export interface FuelEconomyReading {
  /** Litres per 100 km in town, on the road, or as a single combined figure, as the advert states them. */
  city: number | null;
  highway: number | null;
  combined: number | null;
  /** The one figure to compare cars by: combined, else the mean of town and road, else the one stated. */
...
// No car on sale in Uruguay does under 5 km/l or over 35 on fuel (20 to 2,9 L/100 km); outside that it
// is another number.
const MIN_KML = 5;
const MAX_KML = 35;
const CONTEXT = 45;
...
const litresOf = (kmPerLitre: number): number => 100 / kmPerLitre;
...
export function readFuelEconomy(text: string): FuelEconomyReading | null {
  ...
  // Every figure is held in km per litre while it is read (the unit sellers write), unrounded.
  const figures: Array<{ index: number; end: number; kml: number }> = [];
  KM_PER_LITRE.lastIndex = 0;
  let match: RegExpExecArray | null;
  while ((match = KM_PER_LITRE.exec(flat))) figures.push({ index: match.index, end: match.index + match[0].length, kml: numberOf(match[1]!) });
  LITRES_PER_100.lastIndex = 0;
  while ((match = LITRES_PER_100.exec(flat))) {
    const litres = numberOf(match[1]!);
    if (litres > 0) figures.push({ index: match.index, end: match.index + match[0].length, kml: 100 / litres });
  }
  ...
  for (const figure of figures) {
    ...
    if (figure.kml < MIN_KML || figure.kml > MAX_KML) continue;
    const kind = kindBefore(lead);
    if (found[kind] === null) found[kind] = figure.kml;
  }

  const { city: cityKml, highway: highwayKml, combined: combinedKml } = found;
  let city = cityKml === null ? null : litresOf(cityKml);
  let highway = highwayKml === null ? null : litresOf(highwayKml);
  ...
  // km/l label. Small backwards pairs ARE the litres; anything else is dropped.
  if (cityKml !== null && highwayKml !== null && highwayKml < cityKml) {
    if (cityKml < 10 && highwayKml >= litresOf(MAX_KML)) [city, highway] = [cityKml, highwayKml];
    else city = highway = null;
  }
  const combined = combinedKml === null ? null : litresOf(combinedKml);
  const litersPer100Km = combined ?? (city !== null && highway !== null ? (city + highway) / 2 : city ?? highway);
  if (litersPer100Km === null) return null;
  const rounded = (value: number | null): number | null => (value === null ? null : round1(value));
  return { city: rounded(city), highway: rounded(highway), combined: rounded(combined), litersPer100Km: round1(litersPer100Km) };
}
 ...
 * the same model and engine state (or, failing that, the same model; or, coarse, the same fuel and
 * displacement), with at least three sellers behind it (ten for the displacement). One figure per
 * seller: a dealer pastes the same template on every car of a model, and ten copies of one guess are
...
export function attachFuelEconomy<T extends CarListing>(listings: readonly T[]): Array<T & { fuelEconomy: CarFuelEconomy | null }> {
  ...
  listings.forEach((listing, index) => {
    ...
    for (const key of [anyFuelKey(listing), modelKey(listing), engineKey(listing), classKey(listing)])
      if (key) add(key, seller, reading.litersPer100Km);
  });
  ...
  return listings.map((listing, index) => {
    ...
    return {
      ...listing,
      fuelEconomy: { city: null, highway: null, combined: null, litersPer100Km: pick.value, basis: pick.basis, sellers: pick.sellers },
    };
  });
}
