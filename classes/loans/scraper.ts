// Daily lender-TEA scraper (HTTP only). Each parser extracts the representative advertised TEA
// from a lender's public rate page using a page-specific anchor, then a plausibility guard rejects
// values outside a sane band for Uruguayan consumer credit. PURE (string -> result) so parsers are
// unit-tested against captured HTML fixtures. Lenders whose rates are behind a JS/quote form are
// NOT here; their catalogue seed values stand.
//
// Verbatim port of app/server/utils/loanScraper.ts, with `fetch` swapped for `axios` — this runs
// as a pm2 cron job (sync_loans.ts), not inside Nitro, so there is no global $fetch/fetch.
 ...
 * advertised TEA (%), or `null` if its anchor is not found. Keyed by lender `id` (matches
 * classes/loans/catalog.ts's LOAN_LENDERS).
 *
 ...
 */
export const TEA_PARSERS: Record<string, { url: string; extract: (text: string) => number | null }> = {
  ...
  cash: {
    ...
    extract: (text) => {
      const m = text.match(/mayor\s+o\s+igual\s+a\s+10\.000\s+UI\s*[–-]\s*(\d{1,3}(?:[.,]\d{1,2})?)\s*%/);
      return m ? toNum(m[1]!) : null;
    },
  },
};
