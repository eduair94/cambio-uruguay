...
const API_BASE = (
  process.env.RETAIL_ML_API ||
  process.env.CHAIR_ML_API ||
  "http://104.234.204.107:9656/mercadolibre"
).replace(/\/+$/, "");
const PAGE_SIZE = 50;
const MAX_PAGES_PER_QUERY = Number(process.env.RETAIL_ML_MAX_PAGES || process.env.CHAIR_ML_MAX_PAGES || 4);
...
interface MlScan {
  ...
  spec: CategorySpec;
  /** A category page is already narrow, so the title filter there is the laxer one. */
  fromCategory: boolean;
}

/**
 * Text searches and category scans for every spec, interleaved so that a run cut short by
 * {@link maxScans} loses the tail of the list rather than one whole category. Callers order the
 * specs by how much they matter; here we only make sure the first query of the last category
 * outranks the eighth query of the first.
 */
function planScans(specs: readonly CategorySpec[]): MlScan[] {
  const perSpec = specs.map((spec) => {
    const scans: MlScan[] = (spec.mlQueries ?? []).map((q) => ({ params: { q }, spec, fromCategory: false }));
    for (const category of spec.mlCategories ?? []) {
      scans.push({ params: { q: spec.mlCategoryQuery ?? spec.mlQueries?.[0] ?? "", category }, spec, fromCategory: true });
    }
    return scans;
  });
  const plan: MlScan[] = [];
  for (let round = 0; ; round++) {
    const slice = perSpec.map((scans) => scans[round]).filter((scan): scan is MlScan => Boolean(scan));
...
export async function harvestMercadoLibre(
  specs: readonly CategorySpec[],
  maxScans = Number(process.env.RETAIL_ML_MAX_SCANS || 0) || Infinity
  ...
  const plan = planScans(specs);
  const budgeted = plan.slice(0, maxScans);

  for (const scan of budgeted) {
    for (let page = 0; page < MAX_PAGES_PER_QUERY; page++) {
      const results = await searchPage({ ...scan.params, offset: String(page * PAGE_SIZE) });
      pages++;
      ...
      for (const result of results) {
        ...
        const accept = scan.fromCategory
          ...
          : (title: string) => scan.spec.accept(title);
        if (!accept(listing.title, listing.attributes)) {
          rejected++;
          continue;
        }
        listing.attributes.CATEGORY_SPEC = scan.spec.key;
  ...
  const skipped = plan.length - budgeted.length;
  return {
    listings: [...byId.values()],
    ok: reachable,
    note: reachable
      ? `${pages} páginas, ${rejected} descartadas por título${skipped ? `, ${skipped} búsquedas fuera de presupuesto` : ""}`
