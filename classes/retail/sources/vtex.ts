...
//     words in both forms and the category spec does the narrowing.
...
const PAGE_SIZE = 50;
const MAX_PAGES = 8;
/** `ft=` takes one token, so terms are the unit of cost here exactly as in the WooCommerce adapter. */
const MAX_QUERIES = Number(process.env.RETAIL_VTEX_MAX_QUERIES || 24);
...
export async function harvestVtexStore(
  ...
  for (const query of queries) {
    for (let page = 0; page < MAX_PAGES; page++) {
      ...
      for (const product of products) {
        ...
        const spec = specs.find((candidate) => candidate.accept(title, contextOf(product)));
        if (!spec) continue;
  ...
  return {
    ...
    ok: reachable,
    note: reachable
      ? `${scanned} productos revisados, ${byId.size} aceptados en ${currency}${noPrice ? `, ${noPrice} sin precio publicado` : ""}${unexpected ? ` (se esperaba ${store.expectCurrency})` : ""}`
