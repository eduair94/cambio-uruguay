...
// so /en/noticias and /pt/noticias are their own URLs. Pointing all three at the
// bare Spanish path (what a hardcoded string would do) tells Google the translated
// pages are duplicates and drops them from the index.
const canonical = computed(() =>
  locale.value === 'es'
    ? 'https://cambio-uruguay.com/noticias'
    : `https://cambio-uruguay.com/${locale.value}/noticias`
)
...
const breadcrumbJsonLd = computed(() =>
  JSON.stringify({
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 2, name: t('noticias.title'), item: canonical.value },
    ],
  })
)
