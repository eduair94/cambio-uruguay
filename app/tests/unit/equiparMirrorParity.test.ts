...
// there while passing on any machine that has app/.nuxt (2026-09-17). App tests importing root
// modules is the established direction (see aduanaFallback.test.ts).
import { EQUIPAR_CATEGORIES } from '../../../classes/equipar/registry'
import { EQUIPAR_CATEGORY_PAGES } from '../../utils/equiparCategoryPages'

/**
 * `app/utils/equiparCategoryPages.ts` is a hand-kept mirror of this registry: the app's runtime code cannot
...
describe('app-registry parity: categorías de equipar', () => {
  ...
  it('no le sobran categorías al espejo que el backend ya no tenga', () => {
    ...
    for (const page of EQUIPAR_CATEGORY_PAGES) {
      expect(backendKeys.has(page.key), `"${page.key}" está en el app pero no en el registro`).toBe(
        true
      )
    }
  })
