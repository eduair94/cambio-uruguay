...
test('bands, applied geography and browser Back remain shareable; active mode never clears them', async ({
  ...
  await expect(page).toHaveURL(bandUrl)
  await expect(ui.locator('.budget-chips')).toContainText('Cordón')
  await page.keyboard.press('Escape')
  await expect(reopened).toBeHidden()
  await expect(page.getByTestId('budget-filter-trigger')).toBeFocused()
  const cancelled = await openFilters(page)
  await expect(cancelled).toContainText('Montevideo')
  await expect(cancelled).toContainText('Cordón')
  await cancelled.getByRole('button', { name: 'Limpiar', exact: true }).click()
  await cancelled.getByRole('button', { name: 'Ver resultados', exact: true }).click()
