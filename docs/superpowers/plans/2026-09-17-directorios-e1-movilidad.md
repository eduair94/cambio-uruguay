...
- [ ] pm2: `currency-movilidad` (`dist/sync_movilidad.js`, `cron_restart: "47 15 * * *"`) y `currency-movilidad-hourly` (`args: "--fast"`, `cron_restart: "7 * * * *"`), `autorestart: false`, fork, `log_date_format`; comentario con los horarios de los otros consumidores del puente; ambos en `OTHER_APPS`.
