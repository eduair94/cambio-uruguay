...
The 169 failures are all in `/sucursales/*`, `/dolar/*`, `/casa/*` — three dynamic route families this plan never touched (no page in those directories was in Part A's scope). Isolated retesting shows they correlate with `nuxt dev`'s own cold-compile/resource fragility under concurrent load (first-hit Vite compilation of an unvisited dynamic route is slow; hitting ~170 of them at once destabilized the dev process — OOM once, `503 Dev server is unavailable` after a restart), not a code defect in this branch. This matches this project's already-documented dev-mode fragility (memory `lightmode-contrast-audit`: "dev cold-compiles ~80s/page ... run against a **prod** build" for full sweeps). A full 0-failures sweep needs a production build or the deployed site, not local `nuxt dev` — `check:og-images` already supports this via `BASE=<url>`.

**Confirmed against production** (`BASE=https://cambio-uruguay.com npm run check:og-images`, post-fix-review): **697/706 OK**. This closes the "are the 169 dev failures real" question — nearly all were local dev-mode artifacts, not a real gap; `/sucursales`, `/dolar`, `/casa` all render correctly in production save for 9 isolated `HTTP 502`s (`/historico/*` ×6, `/dolar/paysandu`, `/casa/cambio_sir`, `/historico/cambio_rynder/EUR`) — a small, separate, pre-existing issue (502 suggests a reverse-proxy/CDN timeout on a cold satori render for those specific routes, not a missing-OG-wiring problem) worth its own follow-up ticket, not part of this plan's scope.

**Post-review fix (commit 0a894d7):** the final whole-branch review flagged that `checkPath`'s fetch had no timeout, so a hung/cold-compiling route could wedge a pool worker indefinitely (exactly what happened during the local dev-mode verification above). Fixed with `AbortSignal.timeout(15000)`; also made `CONCURRENCY` overridable via `OG_CONCURRENCY` env var.
