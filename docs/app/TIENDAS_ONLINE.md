...
| `age` | `classes/stores/signals/age.ts` | primera captura en la Wayback Machine y certificado TLS más antiguo en crt.sh, consultados los dos siempre; gana la fecha más temprana, pero **Wayback manda**: si Wayback no contesta (ni tras un reintento) la señal entera queda `undefined`, sin importar lo que diga crt.sh — su fecha sola subestima sistemáticamente una tienda vieja. "Wayback" acá son DOS endpoints en orden (fix round F2, item H): primero el CDX (`web.archive.org/cdx/search/cdx`); recién si ese falló del todo, la API de disponibilidad (`archive.org/wayback/available?...&timestamp=19900101`, otro host, para que una caída puntual del CDX no tumbe también al respaldo) — pedir la captura "más cercana" a 1990 la hace degenerar en la MÁS ANTIGUA archivada. Verificado en vivo el 2026-09-17 para tiendainglesa.com.uy con el CDX devolviendo 503: el respaldo contestó 2001-02-01. Cualquiera de los dos que conteste se publica igual como `source: "wayback"`. Sólo si Wayback (cualquiera de los dos) contestó "nada" vale la fecha de crt.sh sola ("primer certificado HTTPS"). El `since` guardado sólo puede volverse más temprano con el tiempo, nunca más tardío (`classes/stores/profile.ts` `mergeAge`) | `domain` y `kind === "tienda-uy"` |
...
| Wayback Machine CDX | `age` (decide si la señal se publica) | timeouts/5xx, o una página HTML "Internet Archive: Temporarily Offline" en vez de JSON bajo carga; un reintento con pausa antes de darla por caída — y si sigue caída, se prueba la API de disponibilidad (item H, fila de abajo) antes de recién ahí declarar TODA la señal `age` `undefined` esa semana, aunque crt.sh haya contestado |
| Wayback Availability API (item H, sólo si el CDX ya falló) | `age` | mismo reintento con pausa; cualquier cuerpo que no sea EXACTAMENTE la forma documentada (sin `url` de nivel superior, `closest.available` que no sea literalmente `true`, timestamp no numérico) cuenta como fallo, nunca como "vacío confirmado" — misma regla que el cuerpo no-array del CDX |
| crt.sh | `age` (sólo cuando algún Wayback contestó) | timeouts/5xx bajo carga; el mismo reintento con pausa. Su fecha sola nunca sostiene la señal si ningún Wayback confirmó por su cuenta (ver las dos filas de arriba) |
