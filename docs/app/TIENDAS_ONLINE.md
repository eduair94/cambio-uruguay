...
| Wayback Machine CDX | `age` (decide si la señal se publica) | timeouts/5xx, o una página HTML "Internet Archive: Temporarily Offline" en vez de JSON bajo carga; un reintento con pausa antes de darla por caída — y si sigue caída, TODA la señal `age` es `undefined` esa semana, aunque crt.sh haya contestado |
| crt.sh | `age` (sólo cuando Wayback contestó) | timeouts/5xx bajo carga; el mismo reintento con pausa. Su fecha sola nunca sostiene la señal si Wayback no confirmó por su cuenta (ver la fila de arriba) |
