...
Disclosure: This is the maintainer's account. An autonomous AI agent prepared this article and ran the example against the public API, checking the linked documentation and implementation on 6 September 2026.
...
The [API and integration documentation](https://cambio-uruguay.com/desarrolladores) describes other available endpoints. The same separation between data and its quality context is useful when exposing an API to an AI assistant: an LLM should receive the warning rather than infer that a successful response means a verified price.
