...
Se confirmaron **siete grupos conflictivos en la captura inicial**, sin afirmar que sean los únicos ni extrapolar una tasa al índice. El caso más pequeño reúne dos unidades distintas de Plutarco 3978. El mayor revisado reúne 35 ofertas bajo una propiedad de Cordón, con direcciones públicas incompatibles. Un grupo legado sin barrio conserva casas y apartamentos de Villa García, Goes y La Unión. La revisión posterior de las 07:40 UTC, documentada al final y separada de estas métricas, confirmó un octavo grupo en San Luis.
...
- `.sdd-rental-confirmed-conflicts.json`: conserva las siete keys iniciales, sus 65 IDs vigentes completos y nueve pares negativos; el apéndice posterior añade San Luis con dos IDs y un par negativo. En total son ocho keys y diez pares, de dos capturas distintas. Especifica cuáles ofertas siguen sin asignación demostrada. Gabriel Pereira, Plutarco y San Luis tienen una partición completa demostrada. Incluye hashes de ambas capturas y no ejecuta cambios.
...
## Revisión posterior de las uniones con identidad v1 — 07:40 UTC

Esta lectura es independiente de la auditoría inicial de las 06:31 UTC y no reemplaza sus conteos. Se consultó la **APP remota de producción**, mediante SSH a `gsearch` y una conexión aislada que tomó `APP_MONGO_URI` del entorno remoto; nunca se utilizó el `MONGO_URI` del backend. Una comprobación preliminar con `app/.env` local había devuelto una copia antigua, con metadato del 20 de agosto y ninguna oferta con identidad v1. Se descartó esa copia como evidencia del índice actual.

La lectura remota de **2026-09-05 07:40:03.125 UTC** encontró exactamente **dos grupos de dos ofertas** al exigir `offers.length > 1` e `identity.version === 1` en todas sus ofertas. El metadato remoto indicaba generación a **07:19:18.826 UTC** y **25.048 propiedades**; este último valor procede del metadato, no de un nuevo recuento completo de documentos. La selección no audita otras propiedades ni garantiza que el estado posterior a la reparación sea idéntico. Las tres fichas InfoCasas consultadas a 07:40:33–34 UTC respondieron 200 y contenían el ID solicitado en `__NEXT_DATA__.props.pageProps.data.id`.

### C8. San Luis: segundo y cuarto apartamento del mismo conjunto

**Key observada:** `canelones-san-luis-rincon-14titov`. **IDs completos:** `infocasas:194165703` y `infocasas:194171253`. Ambas identidades v1 conservaban Rincón 1900, San Luis, Canelones, apartamento de un dormitorio, un baño, 30 m² y las mismas coordenadas. Los precios eran UYU 15.900 y UYU 17.000. Ninguna identidad almacenada distinguía la posición de la unidad dentro del conjunto.

La primera ficha dice **«Es el segundo de 4 apartamentos.»**; la segunda, **«Es el cuarto de 4 apartamentos.»**. Son designaciones explícitas de apartamentos diferentes, no pisos deducidos. La segunda también ofrece patio propio y la posibilidad de incorporar otro dormitorio con un precio diferente. El dato estructurado `floor` es `null` en las dos fichas y `floorsCount` es cero; esos campos no contienen la designación que resuelve este caso. Los códigos comerciales son `UY.42.14.9.63.V7` y `UY.42.14.9.64.V7`, respectivamente. [IC194165703](https://www.infocasas.com.uy/alquiler-apartamento-1-dor-amueblado-en-san-luis/194165703), [IC194171253](https://www.infocasas.com.uy/alquiler-apartamento-1-2-dor-amueblado-san-luis/194171253).

Las portadas originales muestran interiores distintos: paredes rosadas y techo de chapa en el primero; paredes blancas, ventilador de techo y otra distribución de ventanas y mobiliario en el segundo. Esto respalda la lectura, pero **el fundamento decisivo del par negativo son las designaciones segundo/cuarto**. Las fotografías diferentes, por sí solas, también podrían representar habitaciones distintas de una misma vivienda. El conflicto queda confirmado y sus dos ofertas no deben compartir una propiedad. La dirección, las dimensiones y la proximidad de precio no anulan esa evidencia.
...
La ficha InfoCasas, cuyo ID sí se verificó, describe piso 8, contrafrente, 90 m², tres dormitorios y garaje; su campo estructurado `floor` es `null`. **No se recuperó el detalle de Mercado Libre para contrastar por separado su descripción ni una designación de unidad.** El conjunto de similitudes respalda una republicación probable, pero una fotografía del edificio puede reutilizarse en varios apartamentos y un título puede proceder de una plantilla. Este caso no se incluye en el manifiesto de conflictos confirmados ni se presenta como prueba suficiente para una unión automática. Mantenerlo separado por falta de unidad explícita es una decisión conservadora, no una afirmación de que sean viviendas distintas. [InfoCasas194088031](https://www.infocasas.com.uy/alquiler-apto-rambla-de-malvin-3d-1b-y-garaje/194088031), [URL original de Mercado Libre MLU1477297794](https://apartamento.mercadolibre.com.uy/MLU-1477297794-alquiler-apto-rambla-de-malvin-3d-1b-y-garaje-_JM).

### Evidencia de esta revisión y límites

- `.sdd-rental-v1-matches-remote.json`: las dos filas y sus identidades individuales, proyección de lectura remota con hora y metadato. Su hash se incorpora como captura complementaria al manifiesto.
- `.sdd-rental-v1-matches-public.json`: tres detalles públicos con ID validado, hora, URL y atributos originales seleccionados.
- `.sdd-rental-v1-match-photos.png`: comparación visual de las cuatro portadas originales, capturada a **07:41:56.821 UTC**. El navegador utilizado quedó cerrado.

La revisión fue sólo de lectura: no escribió Mongo, no ejecutó sincronizaciones y no modificó el agrupador. La muestra posterior cubre los dos grupos que cumplían el predicado en ese instante; no mide precisión o exhaustividad del resto del índice. **Identidad v1 significa que existen atributos propios del aviso, no que la equivalencia física entre dos avisos esté demostrada.** San Luis muestra que la unidad puede estar sólo en una descripción y quedar fuera de la identidad resumida. Malvín muestra el límite opuesto: la ausencia de contradicciones y el material compartido no completan por sí solos la prueba de una unidad específica.
