...
import { rentalEligibility } from '../../classes/rentals/eligibility'

const home = (extra: Record<string, unknown> = {}) => ({
  title: 'Apartamento', description: '', currency: 'UYU' as const, price: 10500,
  propertyType: 'apartamento', ...extra,
})
...
describe('explicit tenancy guarantee as own peso-rental evidence', () => {
  ...
  [
    ...
    expect(rentalEligibility(home({ title, description, price }))).toEqual({ eligible: true, reasons: [] })
  })
  it.each([
  ...
  it('does not establish a monthly USD price', () => {
    expect(rentalEligibility(home({ currency: 'USD', price: 250, description: 'Garantías Anda, Porto o Sura.' })).reasons).toContain('no_rental_evidence')
  })
  it.each([
...
describe('structured own guarantees and transfer prices', () => {
  it('uses the same advert guarantee field when its description is empty', () => {
    expect(rentalEligibility(home({ description: '', guaranteeText: 'Anda, Porto Seguros o Sura.' })).eligible).toBe(true)
    expect(rentalEligibility(home({ description: '', guaranteeText: 'No acepta Anda' })).reasons).toContain('no_rental_evidence')
    expect(rentalEligibility(home({ currency: 'USD', price: 250, guaranteeText: 'Anda, Porto o Sura' })).reasons).toContain('no_rental_evidence')
  })
  it('retains an independent own guarantee clause after broker advertising', () => {
    expect(rentalEligibility(home({ description: 'Somos corredores de Porto. Garantías Anda, Contaduría o Sura.' })).eligible).toBe(true)
  })
  it.each(['Traspaso. Garantías Porto Seguro, ANDA o Contaduría.', 'Derecho de cesión $10500. Garantías Anda.', 'Derecho de llave. Alquiler anual $10500.'])('does not treat an ambiguous transfer amount as monthly rent: %s', description => {
    expect(rentalEligibility(home({ description })).reasons).toContain('ambiguous_transfer')
  })
  it.each(['Traspaso. Alquiler mensual $10500. Garantías Anda.', 'Traspaso. Alquiler $10500 por mes.', 'No es traspaso. Garantías Anda.'])('accepts explicit monthly rent or a negated transfer: %s', description => {
    expect(rentalEligibility(home({ description })).eligible).toBe(true)
  })
  it.each(['USD', 'U$S', 'US$'])('preserves explicitly priced monthly USD rent in a transfer: %s', currency => {
    expect(rentalEligibility(home({ currency: 'USD', price: 300, description: 'Traspaso. Alquiler mensual ' + currency + '300.' })).eligible).toBe(true)
  })
  it('still rejects USD guarantees without a period and conflicting monthly amounts', () => {
    expect(rentalEligibility(home({ currency: 'USD', price: 300, description: 'Traspaso. Garantías Anda.' })).reasons).toContain('no_rental_evidence')
    expect(rentalEligibility(home({ description: 'Traspaso. Alquiler mensual $12000.' })).reasons).toContain('price_conflict')
  })
})
